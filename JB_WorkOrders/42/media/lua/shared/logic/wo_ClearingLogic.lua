...
ClearingLogic.ClearRegistry = {
    Tree = {
        ...
        action = function(playerObj, square)
            ActionPlayer.addToQueue(playerObj, ISWorldObjectContextMenu.doChopTree, { playerObj, square:getTree() }, { dedupeSquare = true })
        end
    },
    Grass = {
        ...
        action = function(playerObj, square)
            ActionPlayer.addToQueue(playerObj, ISWorldObjectContextMenu.doRemoveGrass, { playerObj, square }, { dedupeSquare = true })
        end
    },
    Bush = {
        isValid = function(square)
            ...
            for objectIndex = 0, square:getObjects():size() - 1 do
                local worldObject = square:getObjects():get(objectIndex)
                if not instanceof(worldObject, "IsoTree")
                    and worldObject:getSprite() and worldObject:getSprite():getProperties()
                    and worldObject:getSprite():getProperties():has(IsoFlagType.canBeCut) then
                    return true
                end
            end
            return false
        ...
        action = function(playerObj, square)
            ActionPlayer.addToQueue(playerObj, ISWorldObjectContextMenu.doRemovePlant, { playerObj, square, false }, { dedupeSquare = true })
        end
    },
    Stump = {
        isValid = function(square)
        ...
        action = function(playerObj, square)
            ...
            end, { playerObj, stumpObj }, { dedupeSquare = true })
        end
    },
    Boulder = {
        isValid = function(square)
        ...
        action = function(playerObj, square)
            ...
            end, { playerObj, boulderObj, config }, { dedupeSquare = true })
        end
    }
}
