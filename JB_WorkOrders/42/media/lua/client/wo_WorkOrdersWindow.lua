...
local KeyConflictDialog = require("wo_HotkeyConflict")
...
local SETTINGS = {
    ...
    { id = "Endurance_Reduction",   kind = "slider", label = "UI_WorkOrders_ModOptions_EnduranceReduction", min = 0, max = 100, step = 5, sandboxVar = "WorkEnduranceReduction", suffix = "%" },
    { id = "Rest_Endurance_Level",  kind = "slider", label = "UI_WorkOrders_ModOptions_RestLevel", min = 0, max = 4, step = 1, sandboxVar = "RestAtEnduranceLevel", suffix = "" },
...
-- in MP the server's sandbox value rules, so show it locked
local function sliderState(setting)
    local locked = isClient() or isServer()
    local value
    if locked then
        value = SandboxVars.JBWorkOrders and SandboxVars.JBWorkOrders[setting.sandboxVar] or 0
    else
        local option = getOpt(setting.id)
        value = option and option:getValue() or 0
    end
    return locked, tonumber(value) or 0
end
...
-- chop a single line down to "..." if it wont fit maxWidth
local function fitText(text, maxWidth, font)
    local textManager = getTextManager()
    if maxWidth <= 0 then return "" end
    if textManager:MeasureStringX(font, text) <= maxWidth then return text end
    while #text > 1 and textManager:MeasureStringX(font, text .. "...") > maxWidth do
...
function Window:clampScroll()
    local viewHeight, contentHeight
    if self.settingsOpen then
        viewHeight = self._settingsViewH or 0
        contentHeight = self.settingsContentH or 0
    else
        viewHeight = self:contentViewH()
...
function Window:prerender()
    ...
    if self._sliderDrag then
        if self.settingsOpen and isMouseButtonDown(0) then
            self:applySliderFromMouse(self._sliderDrag)
        else
            self._sliderDrag = nil
        end
    end
    -- a changed item count means rebuild this shit / andl light now too!
    if not self.settingsOpen then
        local signature = invSignature(player())
        local dark = WorkOrders.isTooDark(player())
...
function Window:onSettingClicked(setting)
    if setting.kind == "slider" then return end
    local option = getOpt(setting.id)
...
function Window:applySliderFromMouse(row)
    local mouseX = self:getMouseX()
    local range = row.max - row.min
    if range <= 0 then return end
    local frac = (mouseX - row.track.x) / row.track.w
    if frac < 0 then frac = 0 elseif frac > 1 then frac = 1 end
    local stepped = row.min + math.floor((frac * range) / row.step + 0.5) * row.step
    if stepped < row.min then stepped = row.min elseif stepped > row.max then stepped = row.max end
    local option = getOpt(row.s.id)
    if option and option:getValue() ~= stepped then
        setOpt(row.s.id, stepped)
    end
end
...
function Window:renderSettings()
    ...
    local headerY = titleHeight + 8
    self:drawText(getText("UI_WorkOrders_Settings"), PAD, headerY, 0.95, 0.82, 0.35, 1, HFONT)
    self:drawRect(PAD, headerY + self.hfontH + 4, self.width - 2 * PAD, 1, 0.4, 0.5, 0.5, 0.5)

    local rowHeight = math.max(26, self.lineH + 12)
    local sliderRowHeight = rowHeight + self.lineH + 8

    local top = headerY + self.hfontH + 12
    local bottom = self:contentBottom()
    local viewHeight = math.max(0, bottom - top)
    self._settingsTop = top
    self._settingsBottom = bottom
    self._settingsViewH = viewHeight

    local contentHeight = 0
    for _, setting in ipairs(SETTINGS) do
        contentHeight = contentHeight + ((setting.kind == "slider") and sliderRowHeight or rowHeight) + 3
    end
    self.settingsContentH = contentHeight
    local maxScroll = math.max(0, contentHeight - viewHeight)
    if self.scrollY < 0 then self.scrollY = 0 end
    if self.scrollY > maxScroll then self.scrollY = maxScroll end
    ...
    local boxSize = 18
    local mouseInView = self:isMouseOver() and mouseY >= top and mouseY <= bottom

    self:setStencilRect(0, top, self.width, viewHeight)

    local currentY = top - self.scrollY

    for _, setting in ipairs(SETTINGS) do
        local option = getOpt(setting.id)
        local thisRowHeight = (setting.kind == "slider") and sliderRowHeight or rowHeight
        local hover = mouseInView and mouseX >= PAD and mouseX <= self.width - PAD and mouseY >= currentY and mouseY <= currentY + thisRowHeight
        if hover then self:drawRect(PAD, currentY, self.width - 2 * PAD, thisRowHeight, 0.14, 1, 1, 1) end

        local rowInfo = { y = currentY, h = thisRowHeight, s = setting }

        if setting.kind == "slider" then
            local locked, value = sliderState(setting)
            local labelShade = locked and 0.55 or 0.9
            local labelAlpha = locked and 0.7 or 1

            local valueText = tostring(math.floor(value + 0.5)) .. (setting.suffix or "")
            local valueWidth = textManager:MeasureStringX(FONT, valueText)
            local valueX = self.width - PAD - 6 - valueWidth
            local labelMaxW = valueX - (PAD + 6) - 8
            self:drawText(fitText(getText(setting.label), labelMaxW, FONT), PAD + 6, currentY + 4, labelShade, labelShade, labelShade, labelAlpha, FONT)
            self:drawText(valueText, valueX, currentY + 4, labelShade, labelShade, labelShade, labelAlpha, FONT)

            local thumbSize = 12
            local scrollGutter = (maxScroll > 0) and 12 or 0
            local trackX = PAD + 6 + thumbSize / 2
            local trackW = (self.width - PAD - 6 - scrollGutter) - (PAD + 6) - thumbSize
            local trackY = currentY + self.lineH + 12
            local trackH = 4
            self:drawRect(trackX, trackY, trackW, trackH, 1, 0.20, 0.24, 0.20)
            self:drawRectBorder(trackX, trackY, trackW, trackH, 1, 0.4, 0.5, 0.4)

            local frac = (setting.max > setting.min) and ((value - setting.min) / (setting.max - setting.min)) or 0
            if frac < 0 then frac = 0 elseif frac > 1 then frac = 1 end
            local thumbX = trackX + frac * trackW - thumbSize / 2
            local thumbY = trackY + trackH / 2 - thumbSize / 2
            local thumbShade = locked and 0.5 or 0.93
            self:drawRect(thumbX, thumbY, thumbSize, thumbSize, 1, thumbShade, locked and 0.5 or 0.84, locked and 0.5 or 0.52)
            self:drawRectBorder(thumbX, thumbY, thumbSize, thumbSize, 1, 0.5, 0.6, 0.5)

            rowInfo.locked = locked
            rowInfo.track = { x = trackX, y = trackY, w = trackW }
            rowInfo.min = setting.min
            rowInfo.max = setting.max
            rowInfo.step = setting.step
            table.insert(self._settingsRows, rowInfo)
            currentY = currentY + thisRowHeight + 3
        else

        local labelX = PAD + 6
        local rightX = self.width - PAD - 6
        local labelY = currentY + (rowHeight - self.lineH) / 2
        local controlY = currentY + (rowHeight - boxSize) / 2

        if setting.kind == "tick" then
            local controlX = rightX - boxSize
            self:drawText(fitText(getText(setting.label), controlX - labelX - 8, FONT), labelX, labelY, 0.9, 0.9, 0.9, 1, FONT)
        ...
        elseif setting.kind == "color" then
            ...
            local swatchX = rightX - swatchWidth
            self:drawText(fitText(getText(setting.label), swatchX - labelX - 8, FONT), labelX, labelY, 0.9, 0.9, 0.9, 1, FONT)
            local color = (option and option:getValue()) or { r = 1, g = 1, b = 1 }
            self:drawRect(swatchX, controlY, swatchWidth, boxSize, 1, color.r, color.g, color.b)
            self:drawRectBorder(swatchX, controlY, swatchWidth, boxSize, 1, 0.6, 0.6, 0.6)
        elseif setting.kind == "key" then
            local keyText
            ...
            self:drawText(fitText(getText(setting.label), (rightX - textWidth) - labelX - 8, FONT), labelX, labelY, 0.9, 0.9, 0.9, 1, FONT)
            self:drawText(keyText, rightX - textWidth, labelY, 0.92, 0.86, 0.5, 1, FONT)
        end
    ...
    if maxScroll > 0 then
        local thumbHeight = math.max(20, viewHeight * (viewHeight / contentHeight))
        local thumbY = top + (viewHeight - thumbHeight) * (self.scrollY / maxScroll)
...
function Window:renderTooltip(item, mouseX, mouseY)
    local lines = {}
    if item.darkBlocked then
        table.insert(lines, getText("UI_WorkOrders_TooDark"))
    elseif item.reqTagKey and not item.enabled then
...
function Window:onMouseDown(mouseX, mouseY)
    ...
    if self.settingsOpen then
        if mouseY >= (self._settingsTop or 0) and mouseY <= (self._settingsBottom or self.height) then
            for _, row in ipairs(self._settingsRows or {}) do
                if row.s.kind == "slider" and not row.locked
                    and mouseX >= PAD and mouseX <= self.width - PAD
                    and mouseY >= row.y and mouseY <= row.y + row.h then
                    self._sliderDrag = row
                    self:applySliderFromMouse(row)
                    self:bringToTop()
                    return true
                end
            end
