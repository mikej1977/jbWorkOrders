...
function MenuModel.computeWindowFlags(playerObj)
    ...
    flags.tooDark = WorkOrders.isTooDark(playerObj)
    return flags
end
...
function MenuModel.buildOrders(playerObj, flags)
    ...
    local function emit(option)
        ...
        local darkBlocked = evalFlags.tooDark and not option.worksInDark
        table.insert(categoryEntry.items, {
            ...
            darkBlocked = darkBlocked,
            enabled = (not darkBlocked) and ((condition == nil) or (condition(playerInv, evalFlags) and true or false)),
            run = runFn,
        })
    end
    ...
    for typeKey, data in pairs(ContainerRegistry.Types) do
        ...
        table.insert(categoryEntry.items, {
            ...
            darkBlocked = flags.tooDark,
            enabled = not flags.tooDark,
