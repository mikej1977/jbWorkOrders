...
local function waterFromSourceStep(playerObj, square, sourceSquare, state)
    ...
    if state.waitingForFill then
        ...
        local dt = now - (state.fillLastMs or now)
        state.fillLastMs = now
        if dt < 0 then dt = 0 elseif dt > 100 then dt = 100 end
        state.fillElapsedMs = (state.fillElapsedMs or 0) + dt
        if state.fillElapsedMs < FILL_WAIT_MS then return false end
    ...
    state.fillElapsedMs = 0
    state.fillLastMs = nil
    return false
end
...
API.addLogic("farmCompost", function(playerObj, worldObjects, area)
    queueOverArea(playerObj, area, Farm.canFertilize, function(playerObj, square)
        local plant = Farm.plantAt(square)
        if not (plant and plant:isAlive()) then return end
        local compost = Farm.findCompost(playerObj)
        if not compost then return end
        if luautils.walkAdj(playerObj, square) then
            local handItem = ISWorldObjectContextMenu.equip(playerObj, playerObj:getPrimaryHandItem(), compost, true)
            ISTimedActionQueue.add(ISFertilizeAction:new(playerObj, handItem, plant, 100))
        end
    end)
end)
