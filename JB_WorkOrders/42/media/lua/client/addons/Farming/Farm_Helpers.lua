...
-- so just reimpl checks on the primitives instead
local Farm = {}
...
function Farm.findCompost(playerObj)
    local inv = playerObj:getInventory()
    local tagged = inv:getFirstTagRecurse(ItemTag.COMPOST)
    if tagged then return tagged end
    return inv:getFirstTypeRecurse("CompostBag")
end
...
function Farm.canWater(square)
    local plant = Farm.plantAt(square)
    return plant ~= nil and plant.state == "seeded" and plant:isAlive() and (plant.waterLvl or 0) < 100
end

function Farm.canFertilize(square)
    local plant = Farm.plantAt(square)
    return plant ~= nil and plant.state == "seeded" and plant:isAlive()
end
