...
API.addMenuOption({
    domain = "Farming", category = "Farming_Tend",
    condition = function(playerInv) return playerInv:containsTagRecurse(ItemTag.COMPOST) or playerInv:containsTypeRecurse("CompostBag") end,
    translate = "UI_Farming_Compost", tooltip = "UI_Farming_Compost_Tooltip",
    reqTag = "UI_Farming_Req_Compost",
    icon = "Item_Compost",
    action = { "SelectArea", "farmCompost" },
})
API.addMenuOption({
    domain = "Farming", category = "Farming_Tend",
...
API.addMenuOption({
    ...
    pickValidator = function(square, playerObj)
        return Farm.findWaterObject(square) ~= nil
            and AdjacentFreeTileFinder.Find(square, playerObj) ~= nil -- need for shoreline tiles
    end,
...
local CURE_OPTIONS = {
    { cure = "Mildew", icon = "Item_TZ_GardeningSprayCan" },
    { cure = "Flies",  icon = "Item_TZ_GardeningSprayCan" },
    { cure = "Slugs",  icon = "Item_SlugRepellent" },
    { cure = "Aphids", icon = "Item_TZ_GardeningSprayCan" },
}
for _, entry in ipairs(CURE_OPTIONS) do
    API.addMenuOption({
        domain = "Farming", category = "Farming_Tend",
        condition = function() return Farm.findCure(localPlayer(), entry.cure) ~= nil end,
        translate = "UI_Farming_Treat_" .. entry.cure,
        tooltip = "UI_Farming_Treat_Tooltip",
        reqTag = "UI_Farming_Req_Cure_" .. entry.cure,
        icon = entry.icon,
        action = { "SelectArea", "farmTreat", entry.cure },
    })
end
